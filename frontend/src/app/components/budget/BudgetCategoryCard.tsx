...
export const mapCategories: {[key: string]: string} = { 
  "Food and Drink": "food",
...
export const mapToCategory = (category: string): string => {
  if (category === null) {
    return "none"
  }
  else{
    const categorySplit = category.split(",")
    if (categorySplit[0] in mapCategories) {
      return mapCategories[categorySplit[0]];
    }
    return "other";
  }
...
const sumByAmounts = (
  ...
  return transactions
    .filter((transaction) => mapToCategory(transaction.category) === category)
